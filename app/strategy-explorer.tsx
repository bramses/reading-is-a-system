...
function strategyDetailImageUrls(strategy: Strategy) {
  return strategy.imageUrls.slice(strategy.imageUrls[0] ? 1 : 0);
}

function strategyHasDetails(
  strategy: Strategy,
  strategyById: Map<string, Strategy>,
) {
  return (
    strategy.body.trim().length > 0 ||
    strategyDetailImageUrls(strategy).length > 0 ||
    strategyLinkGroups(strategy).length > 0 ||
    strategy.pairedWithIds.some((strategyId) => strategyById.has(strategyId))
  );
}
...
function CartIcon({ selected = false }: { selected?: boolean }) {
  return (
    ...
    >
      ...
      <path d="M8 11h8" />
      <path d="M8 15h3" />
      {selected ? <path d="m13 16 2 2 4-5" /> : null}
    </svg>
  );
}
...
export default function StrategyExplorer({ site }: StrategyExplorerProps) {
  ...
  const [readingTimerOpen, setReadingTimerOpen] = useState(false);
  ...
  const readingTimerComplete =
    readingTimerState.completedCount >= READING_TIMER_BLOCK_COUNT;
  const readingTimerStatus = activeReadingTimer
    ? readingTimerPaused
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-7 px-4 py-6 sm:gap-8 sm:px-8 sm:py-8 lg:px-10 print:max-w-none print:gap-0 print:px-0 print:py-0">
        <header className="sticky top-0 z-[80] -mx-4 -mt-6 border-b-2 border-[#22201b] bg-[#f2ede1] print:hidden sm:-mx-8 sm:-mt-8 lg:-mx-10">
          <nav className="riso-topscroll mx-auto flex max-w-6xl items-center gap-2 overflow-x-auto px-4 py-3 sm:px-8 lg:px-10">
            ...
            <Link
              aria-label={`Open checklist with ${cartStrategyIds.length} selected strategies`}
              className="riso-nav-chip ml-auto"
              href="/checklist"
            >
              <CartIcon selected={cartStrategyIds.length > 0} />
              Checklist
              <span className="inline-flex min-w-6 items-center justify-center bg-[var(--accent)] px-1.5 py-0.5 text-white">
                {cartStrategyIds.length}
              </span>
            </Link>
          </nav>
        </header>
          ...
          >
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="max-w-2xl">
                ...
                <p className="mt-3 text-sm leading-6 text-[#4a463c]">
                  Choose a short reading block, open your reader, and keep the
                  session moving. Use the mini game when you want a strategy
                  prompt before you start.
                </p>
                <p
                  aria-live="polite"
                  className="riso-mono mt-3 text-sm font-semibold uppercase text-[#315d4c]"
                >
                  {readingTimerStatus}
                </p>
              </div>
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
                <Link
                  className="riso-button text-center"
                  href="/strategy-game"
                >
                  Mini game
                </Link>
                <button
                  aria-controls="reading-timer-controls"
                  aria-expanded={readingTimerOpen}
                  className="riso-button"
                  onClick={() => setReadingTimerOpen((open) => !open)}
                  type="button"
                >
                  {readingTimerOpen ? "Hide timer" : "Show timer"}
                </button>
              </div>
            </div>

            <div
              className="mt-4 border-t-2 border-dashed border-[#22201b]/30 pt-4"
              hidden={!readingTimerOpen}
              id="reading-timer-controls"
            >
        ...
        <section
          ...
          className="riso-panel p-4 print:hidden sm:p-5"
        >
          ...
          {strategyRows.map((row) => (
            ...
            >
              {row.items.map(({ strategy, index }) => {
                const hasDetails = strategyHasDetails(strategy, strategyById);
                const expanded = hasDetails && expandedId === strategy.id;
                const shouldPulseDetailsButton =
                  hasDetails &&
                  index === 0 &&
                  !detailsButtonInteracted &&
                  !expanded;
                ...
                const detailImageUrls = strategyDetailImageUrls(strategy);
                    ...
                    <div className="p-4 sm:p-5">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        ...
                        <button
                          aria-label={
                            inCart
                              ? `Remove ${strategy.title} from checklist`
                              : `Add ${strategy.title} to checklist`
                          }
                          aria-pressed={inCart}
      ...
      <div className="mx-auto max-w-3xl px-4 py-5">
        {starterPackStrategies.length === 0 ? (
          ...
          <section className="grid gap-4 pb-28">
            {starterPackStrategies.map((strategy) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    >
                      <CartIcon selected={inCart} />
                      {inCart ? "Remove from checklist" : "Add to checklist"}
                    </button>
                  </div>
                </article>
              );
      ...
      {cartState.strategyIds.length > 0 ? (
        <div className="fixed inset-x-0 bottom-0 border-t-2 border-[#22201b] bg-[#f8f4ea] p-4">
          <Link
            ...
            href="/checklist"
          >
            Open checklist ({cartState.strategyIds.length})
          </Link>
        </div>
      ) : null}
    </main>
...
export function ChecklistPage({ site }: StrategyExplorerProps) {
  ...
  async function copyCartMarkdown() {
    ...
    try {
      ...
      showClipboardStatus("Checklist copied as Markdown.");
    } catch {
      showClipboardStatus("Could not copy checklist.");
    }
  }
    ...
    <main className="riso-app min-h-screen bg-[#f2ede1] text-[#22201b]">
      ...
      <div className="mx-auto max-w-5xl px-4 py-5">
        {cartStrategies.length === 0 ? (
          ...
          <section className="grid gap-5 pb-32">
                ...
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0">
                      ...
                      <p className="mt-2 max-w-3xl text-sm leading-6 text-[#4a463c]">
                        {strategy.subtitle}
                      </p>
                    </div>
                    <button
                      className="riso-button w-full shrink-0 sm:w-auto"
      ...
      {cartStrategies.length > 0 ? (
        <div className="fixed inset-x-0 bottom-0 border-t-2 border-[#22201b] bg-[#f8f4ea]">
          <div className="mx-auto grid max-w-5xl grid-cols-2 gap-2 px-4 py-4">
            <button
              className="riso-button w-full"
